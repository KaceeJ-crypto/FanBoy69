#!/usr/bin/env python3
import os, re, sys, time, hashlib, json, urllib.parse, pathlib, datetime
import feedparser
...
def normalize_url(u: str):
    # strip utm params but keep affiliate params we add later
...
def add_affiliate(u: str, aff_map: dict):
    ...
    if not tag: return u
    qs = urllib.parse.parse_qs(url.query)
    # merge affiliate params into query
...
def main():
    ...
    title = f"{today} • Top {len(items)} finds in your niche"
    slug = slugify(title)
